...
\node[draw,fill=orange!70,minimum width=0.4cm,minimum height=0.4cm] at (9,0) {};
\node[right] at (9.3,0) {Disagree};
...
\subsection{Backend}
We're using the modified version of Zachary Robert Bennett's lll-attack-runner as the backend for our visualization of the LLL and BKZ algorithms. We only use three source codes from lll-attack-runner, which are : lll.ts, bkz.ts, and types.ts. The lll.ts, per its filename, does the LLL algorithm process, and the same goes for bkz.ts. Both source codes needs LLLStep interface from types.ts, and for bkz.ts, it also needs LLLRun from lll.ts. 
...
We will use Next.js, a React-based framework for building web applications, with the goal to create a user-friendly interface as much as we can for our visualization of the LLL and BKZ algorithms. The user will write their own matrix (or copy-and-paste it from any sources), insert needed values (for example, delta value for both LLL and BKZ algorithm and the block size for BKZ algorithm), and the frontend will pass the values inserted by the user into the backend for the core processing. As what we already explained before in the overview section, the frontend's responsibility is mainly for visualization part. And, for the visualization part, we will use D3 for the visualization part, combined with React to create matrices and vectors visualizations.
...
\mbox{}
The SVP is computed through each of the BKZ iterations. However, the program would only show it in the final step. It is displayed within a dedicated panel in LLL and BKZ modules.
...
To measure the quality of Latviz that we have develop, we made an online app usability survey and let the survey participants test the app, by putting the Latviz's access link in the survey form. Since lattice-based cryptography and cryptanalysis algorithms require sufficient understanding of concepts within linear algebra which is usually only taught in colleges, our targeted demographic group of this survey are Indonesian college students and graduates, while the questions in the survey are in the Indonesian language. There are 25 questions related with LLL and BKZ modules in the app (including 2 optional suggestion boxes). 

We use Convenience Sampling method as the sampling method for our research, because we already know that both LLL and BKZ, or lattice reduction algorithms in general, require complex mathematical understanding as the requirement for the learners \cite{16}. Linear algebra in general is only start to be taught in colleges, and the fundamental concepts such as linear system of equations, vectors, and matrices are only start to be taught in high schools (in Indonesia, commonly in SMA). Added with time constraints, using convenience sampling method is a primary and a rational choice for us. We spread the survey link to our relatives, especially our fellow faculty colleagues. 

First, we asked the participants basic questions about their familiarity with fundamental concepts used in Latviz (for example, post-quantum cryptography) as the background survey of the respondents. Then, for the questions related with the usability evaluations of the LLL and BKZ, we use the 5-points Likert's Scale, which ranges from "Strongly Disagree" as the lowest score to "Strongly Agree" as the highest score. We created the survey using Google Forms, and the diagrams of the results based on the response from the participants are created automatically by Google Forms.

\subsection{Questions}
\subsubsection{Demographic Background Questions}
We asked several questions about the respondents' familiarity with cryptography, post-quantum cryptography, lattice-based cryptography, LLL, and BKZ before they trying Latviz for the first time. The answer choices for these questions ranging from "Very familiar" to "Not familiar". 
...
To examine whether these perceptions differed based on prior experience of the survey participants with cryptography, we made a classification of the survey participants based on their self-reported familiarity with cryptography and post-quantum cryptography. We defined the as NC (No Familiarity with Cryptography), LC (Low Familiarity with Cryptography), YC-YPQ (Familiar with Cryptography and Post-Quantum Cryptography), and YC-LPQ (Familiar with Cryptography but Low Post-Quantum Cryptography Knowledge).

\begin{table}[H]
\centering
\caption{Respondent Categories}
\mbox{}
\label{tab:respondentgroups}
\begin{tabular}{|c|c|} 
 \hline
 Categories & Percentage \\ [0.5ex]
 \hline
 NC & 41\% \\
 \hline
 LC & 18.1\% \\
...
\begin{table}[H]
\centering
\caption{Responses to LLL Evaluation Questions}
\label{tab:lllresults}
...
\begin{table}[H]
\centering
\caption{Responses to BKZ Evaluation Questions}
\label{tab:bkzresults}
\begin{tabular}{|p{8cm}|c|c|c|c|c|}
\hline
Statements & SD & D & N & A & SA \\
\hline
...
\begin{tikzpicture}
    \pie[
        radius=1.5,
        rotate=180,
        color={gray!60, blue!60, green!70, orange!70},
    ]
    {18.2/,59.1/,13.6/, 9.1/}
\end{tikzpicture}
\caption{LLL}
...
\caption{Responses regarding whether Latviz was perceived as an effective learning tool for LLL and BKZ algorithms}
\label{fig:effectivetool}
\end{figure}

\FloatBarrier
\section{Evaluation}
Before we closed the survey, we managed to get 22 college students or college graduates as the survey participants. A plurality of all respondents (41\% of them) weren't familiar with cryptography, let alone LLL and BKZ. This proves our previous statements about how niche the LLL and BKZ algorithms are. 

More than 70\% of all respondents reportedly having their understandings of LLL and BKZ algorithms improved after using Latviz's LLL and BKZ visualization modules. This number was taken from the total amount of survey participants who answered "Strongly Agree" and "Agree". Using this response type combination, 72.7\% of all survey participants reported understanding increase of LLL and BKZ after they tested Latviz. And of all UI elements surveyed from both LLL and BKZ visualization modules, for LLL, the most satisfying element is the "Calculation Details" panel (81.9\%), and for BKZ, it's none other than the "Back" and "Next" navigation buttons (81.9\%). 

The less positive responses are found in questions about the roles of the variables used in both algorithms. In particular, just 59.1\% of all respondents understand the roles of the variables used in the LLL algorithm, and for BKZ, it is slightly lower, at just 58.2\%. We think this is natural consider how hard the concepts of LLL (and BKZ) are.
