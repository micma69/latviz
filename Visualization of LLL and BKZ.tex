...
We have $L$, a lattice basis, with $n$ vectors within it. The $L$ contains ${\{v_1, v_2, ....v_n\}}$. Assume $L$ is a "bad" basis, then our goal is to make it more orthogonal and shorter. Then, as the first step, the index pointer $k$ will point at the 2nd vector from the left. 
...
After the initialization, the algorithm processes the second vector with Gram-Schmidt orthogonalization process. As a note, for the first vector ($j=1$), no projection is needed since it has no preceding vectors. 

$$\mathbf{v}_1^* = \mathbf{v}_1$$

\subsubsection{Basis reduction}
\mbox{}
After the Gram-Schmidt orthogonalization process, the algorithm checks the size of the vector, and then reduces it dynamically. The vector $\mathbf{v}_k$ is iteratively updated using the following formula:
$$\mathbf{v}_k \leftarrow \mathbf{v}_k - \lfloor\mu_{k,j}\rceil\mathbf{v}_j$$

Once the size reduction loop is complete, the updated vector is then subjected to the core step of the LLL algorithm - the Lovász Condition checking.

\subsubsection{The Lovasz Condition}
\mbox{}
$$\|\mathbf{v}_k^*\|^2 \ge \left(\delta - \mu_{k,k-1}^2\right)\|\mathbf{v}_{k-1}^*\|^2$$

Should the basis at the current iteration fulfills the Lovasz Condition, it will proceed to check whether all vectors have been examined. If that's the case ($k >= n$), then the algorithm terminates entirely. If not, then the iteration continues and $k$ will point to the next vector, until there is no vector left in the basis. 
...
Since this swap alters the geometric orientation and structural sequence of $L$, the previously calculated Gram-Schmidt vectors and coefficients become invalid from index $k-1$ onwards. Consequently, the algorithm must retreat its iteration counter to re-evaluate the consistency of the newly swapped vectors. Then, the vector index pointer $k$ is re-adjusted using the following step:

\begin{equation}
    k \leftarrow \max(k-1, 2)
\end{equation}

This repositioning of $k$ ensures that the counter shifts backward by one step to re-examine the preceding basis relationship, while the $\max$ function serves as a logical guardrail to prevent $k$ from accessing indices lower than $2$.
