    ...
    const internal = {
        ...
        keygen: (seed) => {
            ...
            if (spy) {
                ...
                spy.keygen.publicKey = publicKey.slice();
                spy.keygen.secretKey = secretKey.slice();
                spy.notify('keygen');
            }
        ...
        sign: (secretKey, msg, random, externalMu = false) => {
            ...
            main_loop: for (let kappa = 0;;) {
                ...
                if (spy) {
                    spy.sign.y = y.map(p => p.slice());
                    spy.sign.w = w.map(p => p.slice());
                    spy.sign.w1 = w1.map(p => p.slice());
                ...
                if (spy) {
                    spy.sign.z = cs1.map(p => p.slice());  // cs1 is z at this point
