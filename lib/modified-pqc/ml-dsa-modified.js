...
const polyGetNormInf = (p) => {
    let max = 0;
    for (let i = 0; i < N; i++) {
        const val = Math.abs(smod(p[i]));
        if (val > max) max = val;
    }
    return max;
};
...
function getDilithium(opts) {
    ...
    const internal = {
        ...
        keygen: (seed) => {
            ...
            for (let i = 0; i < K; i++) {
                ...
                NTT.decode(t);
                const withS2 = polyAdd(t, s2[i]);
                if (spy) spy.keygen.t[i] = withS2.slice();
        ...
        sign: (secretKey, msg, random, externalMu = false) => {
            ...
            main_loop: for (let kappa = 0;;) {
                ...
                if (spy) iteration.y = y.map(p => p.slice());
                ...
                if (spy) iteration.w = w.map(p => p.slice());

                const w1 = w.map((j) => j.map(HighBits));
                if (spy) iteration.w1 = w1.map(p => p.slice());
                ...
                if (spy) {
                    iteration.cTilde = cTilde.slice();
                    spy.sign.cTilde = cTilde.slice();
                }

                const cHat = NTT.encode(SampleInBall(cTilde));
                if (spy) iteration.c = cHat.slice();

                const cs1 = s1.map((i) => MultiplyNTTs(i, cHat));
                
                // STORE z BEFORE checking rejection
                for (let i = 0; i < L; i++) {
                    polyAdd(NTT.decode(cs1[i]), y[i]);
                }
                if (spy) iteration.z = cs1.map(p => p.slice());
                
                // Now check rejection
                let zRejected = false;
                for (let i = 0; i < L; i++) {
                    const normInf = polyGetNormInf(cs1[i]);
                    if (spy) iteration.zNormInf = normInf;
                    if (polyChknorm(cs1[i], GAMMA1 - BETA)) {
                        iteration.rejected = true;
                        iteration.reason = `||z||∞ = ${normInf} ≥ ${GAMMA1 - BETA}`;
                        zRejected = true;
                        break;
                    }
                }
                ...
                if (zRejected) {
                    if (spy && spy.sign.iterations) spy.sign.iterations.push(iteration);
                    continue main_loop;
                }

                let cnt = 0;
                const h = [];
                let r0Rejected = false;
                let ct0Rejected = false;
                
                // STORE r0 BEFORE checking rejection
                const allR0 = [];
                const allCt0 = [];
                ...
                for (let i = 0; i < K; i++) {
                    ...
                    allCt0.push(ct0.slice());
                }
                
                if (spy) {
                    iteration.r0 = allR0;
                    iteration.ct0 = allCt0;
                }
                
                // Now check rejections
                for (let i = 0; i < K; i++) {
                    const r0NormInf = polyGetNormInf(allR0[i]);
                    if (spy) iteration.r0NormInf = r0NormInf;
                    
                    if (polyChknorm(allR0[i], GAMMA2 - BETA)) {
                        iteration.rejected = true;
                        iteration.reason = `||r0||∞ = ${r0NormInf} ≥ ${GAMMA2 - BETA}`;
                        r0Rejected = true;
                        break;
                    }
                    
                    const ct0NormInf = polyGetNormInf(allCt0[i]);
                    if (spy) iteration.ct0NormInf = ct0NormInf;
                    
                    if (polyChknorm(allCt0[i], GAMMA2)) {
                        iteration.rejected = true;
                        iteration.reason = `||c·t0||∞ = ${ct0NormInf} ≥ ${GAMMA2}`;
                        ct0Rejected = true;
                        break;
                    }
                }
                
                if (r0Rejected || ct0Rejected) {
                    if (spy && spy.sign.iterations) spy.sign.iterations.push(iteration);
                    continue main_loop;
                }
                
                // Now compute hints using stored values
                for (let i = 0; i < K; i++) {
                    const r0Copy = allR0[i].slice();
                    polyAdd(r0Copy, allCt0[i]);
                    const hint = polyMakeHint(r0Copy, w1[i]);
                    h.push(hint.v);
                    cnt += hint.cnt;
                }
                
                if (spy) iteration.hammingWeight = cnt;
                
                if (cnt > OMEGA) {
                    iteration.rejected = true;
                    iteration.reason = `hamming weight ${cnt} > ${OMEGA}`;
                    if (spy && spy.sign.iterations) spy.sign.iterations.push(iteration);
                    continue;
                }
                ...
                iteration.rejected = false;
                iteration.h = h.map(p => p.slice());
                finalKappa = kappa;
                
                if (spy) {
                    if (spy.sign.iterations) spy.sign.iterations.push(iteration);
                    spy.sign.z = cs1.map(p => p.slice());
                    spy.sign.h = h.map(p => p.slice());
                    spy.sign.kappa = finalKappa;
                    spy.sign.signature = sigCoder.encode([cTilde, cs1, h]).slice();
                    spy.notify('sign');
                }
    ...
    return {
        ...
        verify: (publicKey, msg, sig, ctx = EMPTY) => {
            ...
            const M = getMessage(msg, ctx);
            if (spy) spy.sign.M = M.slice();
            return internal.verify(publicKey, M, sig);
        },
        prehash: (hashName) => ({
            sign: (secretKey, msg, ctx = EMPTY, random) => {
                if (spy) {
